<?php
  ...
  function check_if_product_in_brand($brandId) {
       global $CI;
       $CI->load->model('product_model');
       $products = $CI->product_model->getProductsByBrand($brandId);
       if (!empty($products)) {
            return true;
       } else {
            return false;
  ...
  function get_original_name_for_url($name) {
       return str_replace('_', ' ', strtolower(trim($name)));
  }
  
  function get_url_maker($name) {
       return str_replace(' ', '-', strtolower(trim($name)));
  }
  
  function get_url_remaker($name) {
       return str_replace('-', ' ', strtolower(trim($name)));
  }
?>