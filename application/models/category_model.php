<?php
  ...
  class Category_model extends CI_Model {
       ...
       function getParentCategory($id, $limit = '', $start = '') {
            ...
            if (!empty($resultTmp)) {
                 foreach ($resultTmp as $key => $value) {
                      ...
                      $value['cat_parent'] = $this->getSingleCategory($value['cat_parent']);
                      $result[] = $value;
                 }
            }
            return $result;
       ...
       function getCategoryByName($name, $isParent = false, $parenrId = '') {
            ...
            if ($name) {
                 ...
                 if ($isParent == true) {
                      $this->db->where('cat_parent', 0);
                 } else if(!empty($parenrId)) {
                      $this->db->where('cat_parent', $parenrId);
                 }
                 return $this->db->get()->row_array();
